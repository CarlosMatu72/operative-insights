...
import { useGlosaCases, useGlosaActions, type GlosaCaseRow } from "@/hooks/useGlosaPanel";
...
const Glosa = () => {
  ...
  const canGlosar = (c: GlosaCaseRow) =>
  ...
  const canCorrecciones = (c: GlosaCaseRow & { findings_count?: number }) =>
    ["CORRECCION_PENDIENTE", "EN_CORRECCION"].includes(c.status) && (c.findings_count ?? 0) > 0;
  ...
  const renderTable = (rows: GlosaCaseRow[], emptyMessage: string) => (
              ...
              <TableCell colSpan={13} className="text-center py-16">
                <div className="flex flex-col items-center gap-2 text-muted-foreground">
                  ...
                  <p className="text-sm font-medium">
                    {hasFilters ? "Sin resultados para los filtros aplicados" : emptyMessage}
                  </p>
                  <p className="text-xs">
  ...
  const misCases = cases.filter((c) => c.assigned_glosador_user_id === user?.id);
            ...
            <div className="flex items-end justify-between gap-4 flex-wrap">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {cases.length} trámite{cases.length !== 1 ? "s" : ""}
                </p>
              </div>
              <TabsList>
                <TabsTrigger value="todos">Todos los trámites</TabsTrigger>
                <TabsTrigger value="mias">Mis revisiones ({misCases.length})</TabsTrigger>
              </TabsList>
            </div>

            <TabsContent value="todos" className="space-y-5 mt-0">
              {activeBanner}
              {renderFilters()}
              {renderTable(cases, "Sin trámites en el sistema")}
            </TabsContent>

            <TabsContent value="mias" className="space-y-5 mt-0">
              {activeBanner}
              {renderFilters()}
              {renderTable(misCases, "No tienes trámites asignados")}
            </TabsContent>
          </Tabs>
        ) : (
          <>
            ...
            {activeBanner}
            {renderFilters()}
            {renderTable(cases, "Sin trámites asignados")}
          </>
        )}
      </div>
